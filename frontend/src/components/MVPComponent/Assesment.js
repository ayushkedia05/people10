...
useEffect(() => {
  fetch(`https://api.spoonacular.com/food/ingredients/search?apiKey=63617f30cddd4a178f6f44085ffd035c&query=${data}&number=1`)
  .then((response)=> response.json())
  .then((data)=>{
    console.log(data)
    setId(data.results[0].id)
  
  fetch(`https://api.spoonacular.com/recipes/${data.results[0].id}/nutritionWidget.json?apiKey=63617f30cddd4a178f6f44085ffd035c&includeNutrition=true`).then((response)=>response.json())
  ...
  return (
    ...
    <div className='w-[40%] border-2'>
     ...
     <img src={`https://api.spoonacular.com/recipes/${id}/nutritionWidget.png?apiKey=63617f30cddd4a178f6f44085ffd035c&includeNutrition=true`} alt="sdd" id='spoon'></img>
    </div>
    // </Card>
  )
}
